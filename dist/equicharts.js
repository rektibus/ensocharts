...
function logWarn(api, invalidParam, append) {
  log(
    "%c\u{1F611} klinecharts warning%c %s%s%s",
...
function logError(api, invalidParam, append) {
  log(
    "%c\u{1F61F} klinecharts error%c %s%s%s",
...
// src/common/utils/typeChecks.ts
...
// src/common/utils/compatible.ts
...
function eachFigures(indicator, dataIndex, defaultStyles, eachFigureCallback) {
  ...
  const figures2 = indicator.figures;
  const styles2 = indicator.styles;
  const circleStyles = formatValue(styles2, "circles", defaultStyles.circles);
  const circleStyleCount = circleStyles.length;
  const barStyles = formatValue(styles2, "bars", defaultStyles.bars);
  const barStyleCount = barStyles.length;
  const lineStyles = formatValue(styles2, "lines", defaultStyles.lines);
  ...
  let figureIndex = 0;
  figures2.forEach((figure) => {
    switch (figure.type) {
      case "circle": {
        figureIndex = circleCount;
        const styles3 = circleStyles[circleCount % circleStyleCount];
        defaultFigureStyles = { ...styles3, color: styles3.noChangeColor };
        circleCount++;
        break;
      }
      case "bar": {
        figureIndex = barCount;
        const styles3 = barStyles[barCount % barStyleCount];
        defaultFigureStyles = { ...styles3, color: styles3.noChangeColor };
        barCount++;
        break;
      }
...
var IndicatorImp = class _IndicatorImp {
  ...
  override(indicator) {
    ...
    this.figures = figures2 ?? this.figures;
  }
  ...
  static extend(template) {
    class Custom extends _IndicatorImp {
      constructor() {
        super(template);
      }
    }
...
var averagePrice_default = averagePrice;
...
var awesomeOscillator_default = awesomeOscillator;
...
var bias = {
  ...
  calc: (dataList, indicator) => {
    const { calcParams: params, figures: figures2 } = indicator;
    const closeSums = [];
    return dataList.map((kLineData, i) => {
      const bias2 = {};
...
var bollingerBands_default = bollingerBands;
...
var bullAndBearIndex_default = bullAndBearIndex;

// src/extension/indicator/commodityChannelIndex.ts
  ...
  calc: (dataList, indicator) => {
    ...
    return dataList.map((kLineData, i) => {
      ...
      if (i >= p) {
        ...
        let sum = 0;
        sliceTpList.forEach((tp2) => {
          sum += Math.abs(tp2 - maTp);
        });
...
var commodityChannelIndex_default = commodityChannelIndex;

// src/extension/indicator/currentRatio.ts
var currentRatio = {
  name: "CR",
  shortName: "CR",
...
var currentRatio_default = currentRatio;
...
var differentOfMovingAverage_default = differentOfMovingAverage;
...
var directionalMovementIndex_default = directionalMovementIndex;
    ...
    return dataList.map((kLineData, i) => {
      ...
      if (i > 0) {
        ...
        if (volume2 === 0 || high - low === 0) {
          ...
          const ratio = volume2 / 1e8 / (high - low);
...
var easeOfMovementValue_default = easeOfMovementValue;
...
var exponentialMovingAverage_default = exponentialMovingAverage;

// src/extension/indicator/momentum.ts
var momentum = {
  name: "MTM",
  shortName: "MTM",
  calcParams: [12, 6],
...
var momentum_default = momentum;

// src/extension/indicator/movingAverage.ts
var movingAverage = {
  name: "MA",
  shortName: "MA",
  series: "price",
...
var movingAverage_default = movingAverage;
...
var movingAverageConvergenceDivergence_default = movingAverageConvergenceDivergence;

// src/extension/indicator/onBalanceVolume.ts
var onBalanceVolume = {
  name: "OBV",
  shortName: "OBV",
  calcParams: [30],
...
var onBalanceVolume_default = onBalanceVolume;
...
var priceAndVolumeTrend = {
  ...
  calc: (dataList) => {
    ...
    return dataList.map((kLineData, i) => {
      const pvt = {};
      const close = kLineData.close;
      const volume2 = kLineData.volume ?? 1;
      const prevClose = (dataList[i - 1] ?? kLineData).close;
      let x = 0;
      const total = prevClose * volume2;
      if (total !== 0) {
...
var priceAndVolumeTrend_default = priceAndVolumeTrend;

// src/extension/indicator/psychologicalLine.ts
...
var psychologicalLine_default = psychologicalLine;
...
var rateOfChange_default = rateOfChange;
...
var relativeStrengthIndex_default = relativeStrengthIndex;
...
var simpleMovingAverage_default = simpleMovingAverage;

// src/extension/indicator/stoch.ts
var stoch = {
  name: "KDJ",
  shortName: "KDJ",
  calcParams: [9, 3, 3],
  ...
  figures: [
    {
      ...
      styles: ({ data, indicator, defaultStyles }) => {
        ...
        const color = sar < halfHL ? formatValue(indicator.styles, "circles[0].upColor", defaultStyles.circles[0].upColor) : formatValue(indicator.styles, "circles[0].downColor", defaultStyles.circles[0].downColor);
        return { color };
      }
    }
  ],
...
var stopAndReverse_default = stopAndReverse;
...
var tripleExponentiallySmoothedAverage_default = tripleExponentiallySmoothedAverage;

// src/extension/indicator/volume.ts
...
var volume = {
  ...
  regenerateFigures: (params) => {
    const figures2 = params.map((p, i) => ({ key: `ma${i + 1}`, title: `MA${p}: `, type: "line" }));
  ...
  calc: (dataList, indicator) => {
    ...
    return dataList.map((kLineData, i) => {
      const volume2 = kLineData.volume ?? 0;
      const vol = { volume: volume2, open: kLineData.open, close: kLineData.close };
      params.forEach((p, index) => {
        volSums[index] = (volSums[index] ?? 0) + volume2;
        if (i >= p - 1) {
...
// src/extension/indicator/volumeRatio.ts
var volumeRatio = {
  name: "VR",
  shortName: "VR",
  calcParams: [26, 6],
...
var volumeRatio_default = volumeRatio;

// src/extension/indicator/williamsR.ts
var williamsR = {
  name: "WR",
  shortName: "WR",
  calcParams: [6, 10, 14],
...
var williamsR_default = williamsR;

// src/extension/indicator/index.ts
var indicators = {};
var extensions = [
  averagePrice_default,
  awesomeOscillator_default,
  bias_default,
  bollingerBands_default,
  brar_default,
  bullAndBearIndex_default,
  commodityChannelIndex_default,
  currentRatio_default,
  differentOfMovingAverage_default,
  directionalMovementIndex_default,
  easeOfMovementValue_default,
  exponentialMovingAverage_default,
  momentum_default,
  movingAverage_default,
  movingAverageConvergenceDivergence_default,
  onBalanceVolume_default,
  priceAndVolumeTrend_default,
  psychologicalLine_default,
  rateOfChange_default,
  relativeStrengthIndex_default,
  simpleMovingAverage_default,
  stoch_default,
  stopAndReverse_default,
  tripleExponentiallySmoothedAverage_default,
  volume_default,
  volumeRatio_default,
  williamsR_default
];
extensions.forEach((indicator) => {
...
var OverlayImp = class _OverlayImp {
  constructor(overlay) {
    this.groupId = "";
    this.totalStep = 1;
  ...
  static extend(template) {
    class Custom extends _OverlayImp {
      constructor() {
        super(template);
      }
    }
  ...
  createPointFigures: ({ chart, coordinates, bounding, overlay, yAxis }) => {
    ...
    if (coordinates.length > 0) {
      ...
      if (yAxis?.isInCandle() ?? true) {
        ...
        const indicators2 = chart.getIndicators({ paneId: overlay.paneId });
...
var fibonacciLine_default = fibonacciLine;

// src/extension/overlay/horizontalRayLine.ts
...
var horizontalRayLine_default = horizontalRayLine;

// src/extension/overlay/horizontalSegment.ts
...
var horizontalSegment_default = horizontalSegment;

// src/extension/overlay/horizontalStraightLine.ts
...
var horizontalStraightLine_default = horizontalStraightLine;

// src/common/Eventful.ts
var Eventful = class {
  constructor() {
    this._children = [];
...
// src/component/Figure.ts
var DEVIATION = 2;
var FigureImp = class _FigureImp extends Eventful {
  constructor(figure) {
    super();
  ...
  static extend(figure) {
    class Custom extends _FigureImp {
      checkEventOnImp(coordinate, attrs, styles2) {
        return figure.checkEventOn(coordinate, attrs, styles2);
      }
      drawImp(ctx, attrs, styles2) {
...
function drawLine(ctx, attrs, styles2) {
  let lines = [];
  lines = lines.concat(attrs);
  const { style = "solid", smooth = false, size = 1, color = "currentColor", dashedValue = [2, 2] } = styles2;
  ctx.lineWidth = size;
...
var line = {
  ...
  draw: (ctx, attrs, styles2) => {
    drawLine(ctx, attrs, styles2);
  }
};
var line_default = line;

// src/extension/overlay/parallelStraightLine.ts
  ...
  if (coordinates.length > 1) {
    if (coordinates[0].x === coordinates[1].x) {
      ...
      if (coordinates.length > 2) {
        ...
        const distance4 = coordinates[0].x - coordinates[2].x;
        for (let i = 0; i < count; i++) {
          const d = distance4 * (i + 1);
      ...
      if (coordinates.length > 2) {
        ...
        const distance4 = b - b1;
        for (let i = 0; i < count; i++) {
          const b2 = b + distance4 * (i + 1);
...
var parallelStraightLine_default = parallelStraightLine;

// src/extension/overlay/priceChannelLine.ts
...
var priceChannelLine_default = priceChannelLine;
...
var priceLine_default = priceLine;

// src/extension/overlay/rayLine.ts
...
var rayLine_default = rayLine;

// src/extension/overlay/segment.ts
var segment = {
  name: "segment",
  totalStep: 3,
  needDefaultPointFigure: true,
...
// src/extension/overlay/straightLine.ts
var straightLine = {
  name: "straightLine",
  totalStep: 3,
  needDefaultPointFigure: true,
...
// src/extension/overlay/verticalRayLine.ts
var verticalRayLine = {
...
var verticalRayLine_default = verticalRayLine;

// src/extension/overlay/verticalSegment.ts
var verticalSegment = {
...
var verticalSegment_default = verticalSegment;

// src/extension/overlay/verticalStraightLine.ts
...
var verticalStraightLine_default = verticalStraightLine;

// src/extension/overlay/simpleAnnotation.ts
...
var simpleAnnotation_default = simpleAnnotation;

// src/extension/overlay/simpleTag.ts
var simpleTag = {
  name: "simpleTag",
  totalStep: 2,
  styles: {
  ...
  createYAxisFigures: ({ chart, overlay, coordinates, bounding, yAxis }) => {
    ...
    if (!isValid(text2) && isNumber(overlay.points[0].value)) {
      text2 = formatPrecision(overlay.points[0].value, chart.getSymbol()?.pricePrecision ?? SymbolDefaultPrecisionConstants.PRICE);
    }
    return { type: "text", attrs: { x, y: coordinates[0].y, text: text2, align: textAlign, baseline: "middle" } };
  }
};
var simpleTag_default = simpleTag;

// src/extension/overlay/arrow.ts
...
// src/extension/overlay/circle.ts
...
// src/extension/overlay/triangle.ts
var triangle = {
  name: "triangle",
  totalStep: 4,
  needDefaultPointFigure: true,
...
// src/extension/overlay/parallelogram.ts
var parallelogram = {
  name: "parallelogram",
  totalStep: 4,
  needDefaultPointFigure: true,
...
// src/extension/overlay/fibonacciCircle.ts
...
var fibonacciCircle = {
  ...
  createPointFigures: ({ coordinates }) => {
    if (coordinates.length > 1) {
      const radius = distance2(coordinates[0], coordinates[1]);
...
// src/extension/overlay/fibonacciSegment.ts
...
// src/extension/overlay/fibonacciExtension.ts
...
// src/extension/overlay/fibonacciSpiral.ts
function distance3(p1, p2) {
  const dx = Math.abs(p1.x - p2.x);
  const dy = Math.abs(p1.y - p2.y);
  return Math.sqrt(dx * dx + dy * dy);
}
function rotatePoint2(point, center, angle) {
  const x = (point.x - center.x) * Math.cos(angle) - (point.y - center.y) * Math.sin(angle) + center.x;
  const y = (point.x - center.x) * Math.sin(angle) + (point.y - center.y) * Math.cos(angle) + center.y;
  return { x, y };
}
function extendLine(coords, bounding) {
  if (coords.length > 1) {
    ...
    if (coords[0].x === coords[1].x && coords[0].y !== coords[1].y) {
      end = coords[0].y < coords[1].y ? { x: coords[0].x, y: bounding.height } : { x: coords[0].x, y: 0 };
...
var fibonacciSpiral = {
  ...
  createPointFigures: ({ coordinates, bounding }) => {
    if (coordinates.length > 1) {
      const baseRadius = distance3(coordinates[0], coordinates[1]) / Math.sqrt(24);
      const direction = coordinates[1].x > coordinates[0].x ? 0 : 1;
      const slopeIntercept = getLinearSlopeIntercept(coordinates[0], coordinates[1]);
      let angle;
      if (slopeIntercept) {
      ...
      const p1 = rotatePoint2({ x: coordinates[0].x - baseRadius, y: coordinates[0].y }, coordinates[0], angle);
      const p2 = rotatePoint2({ x: coordinates[0].x - baseRadius, y: coordinates[0].y - baseRadius }, coordinates[0], angle);
      const arcs = [
      ...
      for (let i = 2; i < 9; i++) {
        ...
        const center = rotatePoint2({ x: cx, y: cy }, coordinates[0], angle);
...
// src/extension/overlay/fibonacciSpeedResistanceFan.ts
function extendLine2(coords, bounding) {
  if (coords.length > 1) {
    let end;
    if (coords[0].x === coords[1].x && coords[0].y !== coords[1].y) {
      end = coords[0].y < coords[1].y ? { x: coords[0].x, y: bounding.height } : { x: coords[0].x, y: 0 };
    ...
    if (coordinates.length > 1) {
      ...
      percents.forEach((p) => {
        ...
        fanLines = fanLines.concat(
          extendLine2([p0, { x, y: p1.y }], bounding)
        );
        fanLines = fanLines.concat(
          extendLine2([p0, { x: p1.x, y }], bounding)
        );
...
// src/extension/overlay/fibonacciDiagonal.ts
var fibonacciDiagonal = {
  ...
  createPointFigures: ({ coordinates, overlay }) => {
    ...
    if (coordinates.length > 0) {
      ...
      if (coordinates.length > 1 && points[0].value !== void 0 && points[1].value !== void 0) {
        const percents = [1, 0.786, 0.618, 0.5, 0.382, 0.236, 0];
        const yDif = coordinates[0].y - coordinates[1].y;
        const valueDif = points[0].value - points[1].value;
        percents.forEach((p) => {
...
var fibonacciDiagonal_default = fibonacciDiagonal;

// src/extension/overlay/gannBox.ts
var gannBox = {
  name: "gannBox",
  totalStep: 3,
  needDefaultPointFigure: true,
...
// src/extension/overlay/waves.ts
...
// src/extension/overlay/xabcd.ts
var xabcd = {
  name: "xabcd",
  totalStep: 6,
  needDefaultPointFigure: true,
...
// src/extension/overlay/elliotTripleComboWaves.ts
...
var elliotTripleComboWaves_default = elliotTripleComboWaves;

// src/extension/overlay/headAndShoulders.ts
...
var headAndShoulders_default = headAndShoulders;

// src/extension/overlay/measure.ts
var measure = {
  name: "measure",
  totalStep: 3,
  needDefaultPointFigure: true,
  ...
  createPointFigures: ({ coordinates, overlay, bounding }) => {
    if (coordinates.length > 1) {
      ...
      if (isRight) {
        figures2.push({ type: "line", attrs: { coordinates: [{ x: p1.x - 6, y: center.y - 4 }, { x: p1.x, y: center.y }, { x: p1.x - 6, y: center.y + 4 }] } });
      } else {
        figures2.push({ type: "line", attrs: { coordinates: [{ x: p1.x + 6, y: center.y - 4 }, { x: p1.x, y: center.y }, { x: p1.x + 6, y: center.y + 4 }] } });
      }
      if (isDown) {
        figures2.push({ type: "line", attrs: { coordinates: [{ x: center.x - 4, y: p1.y - 6 }, { x: center.x, y: p1.y }, { x: center.x + 4, y: p1.y - 6 }] }, styles: { color: lineColor } });
      } else {
        figures2.push({ type: "line", attrs: { coordinates: [{ x: center.x - 4, y: p1.y + 6 }, { x: center.x, y: p1.y }, { x: center.x + 4, y: p1.y + 6 }] }, styles: { color: lineColor } });
      }
...
// src/extension/overlay/crossLine.ts
var crossLine = {
  name: "crossLine",
  totalStep: 2,
  needDefaultPointFigure: true,
...
var crossLine_default = crossLine;

// src/extension/overlay/flatTopBottom.ts
var flatTopBottom = {
  name: "faltTopBottom",
  totalStep: 4,
  needDefaultPointFigure: true,
...
var flatTopBottom_default = flatTopBottom;

// src/extension/overlay/disJointChannel.ts
var disJointChannel = {
...
var disJointChannel_default = disJointChannel;

// src/extension/overlay/arcOverlay.ts
var arcOverlay = {
  name: "arc",
  totalStep: 3,
  needDefaultPointFigure: true,
...
var arcOverlay_default = arcOverlay;

// src/extension/overlay/tradingPlan.ts
var tradingPlan = {
  name: "tradingPlan",
  totalStep: 5,
  needDefaultPointFigure: true,
...
var tradingPlan_default = tradingPlan;

// src/extension/overlay/index.ts
var overlays = {};
var extensions2 = [
  // Base overlays
  fibonacciLine_default,
  horizontalRayLine_default,
  horizontalSegment_default,
  horizontalStraightLine_default,
  parallelStraightLine_default,
  priceChannelLine_default,
  priceLine_default,
  rayLine_default,
  segment_default,
  straightLine_default,
  verticalRayLine_default,
  verticalSegment_default,
  verticalStraightLine_default,
  simpleAnnotation_default,
  simpleTag_default,
  // Pro overlays
  arrow_default,
  circle_default,
  ...
  fibonacciCircle_default,
  fibonacciSegment_default,
  fibonacciExtension_default,
  fibonacciSpiral_default,
  fibonacciSpeedResistanceFan_default,
  fibonacciDiagonal_default,
  gannBox_default,
  threeWaves,
  fiveWaves,
  eightWaves,
  ...
  abcd_default,
  xabcd_default,
  elliotTripleComboWaves_default,
  headAndShoulders_default,
  measure_default,
  crossLine_default,
  flatTopBottom_default,
  disJointChannel_default,
  arcOverlay_default,
  tradingPlan_default
];
extensions2.forEach((template) => {
...
var DEFAULT_MIN_TIME_SPAN = 15 * 60 * 1e3;
var StoreImp = class {
  constructor(chart, options) {
    ...
    this._formatter = {
      formatDate: ({
        ...
        timestamp,
        template
      }) => formatTimestampByTemplate(dateTimeFormat, timestamp, template),
      formatBigNumber,
      formatExtendText: (_) => ""
    };
    /**
    ...
    * Whether there are forward and backward more flag
    ...
    const { styles: styles2, locale, timezone, formatter, thousandsSeparator, decimalFold, zoomAnchor } = options ?? {};
    if (isValid(styles2)) {
  ...
  setStyles(value) {
    ...
    if (isArray(styles2?.candle?.tooltip?.legend?.template)) {
      this._styles.candle.tooltip.legend.template = styles2.candle.tooltip.legend.template;
    }
    if (isValid(styles2?.candle?.priceMark?.last?.extendTexts)) {
      ...
      this._styles.candle.priceMark.last.extendTexts.forEach((item) => {
        ...
        if (item.show && updateInterval > 0 && !intervals.includes(updateInterval)) {
          ...
          const timer = setInterval(() => {
            this._chart.updatePane(0 /* Main */, PaneIdConstants.CANDLE);
          }, updateInterval);
  ...
  setOffsetRightDistance(distance4, isUpdate) {
    this._offsetRightDistance = this._scrollLimitRole === "distance" ? Math.min(this._maxOffsetDistance.right, distance4) : distance4;
    this._lastBarRightSideDiffBarCount = this._offsetRightDistance / this._barSpace;
    if (isUpdate ?? false) {
      this._adjustVisibleRange();
      this.setCrosshair(this._crosshair, { notInvalidate: true });
  ...
  setMaxOffsetLeftDistance(distance4) {
    this._scrollLimitRole = "distance";
    this._maxOffsetDistance.left = distance4;
  }
  setMaxOffsetRightDistance(distance4) {
    this._scrollLimitRole = "distance";
    this._maxOffsetDistance.right = distance4;
  }
  ...
  scroll(distance4) {
    ...
    const distanceBarCount = distance4 / this._barSpace;
  ...
  setCrosshair(crosshair, options) {
    ...
    this._crosshair = { ...cr, realX, kLineData, realDataIndex, dataIndex, timestamp: this.dataIndexToTimestamp(realDataIndex) ?? void 0 };
    if (prevCrosshair.x !== cr.x || prevCrosshair.y !== cr.y || prevCrosshair.paneId !== cr.paneId || (forceInvalidate ?? false)) {
      ...
      if (!(notInvalidate ?? false)) {
        this._chart.updatePane(1 /* Overlay */);
      }
    }
  }
  getCrosshair() {
  ...
  _synchronizeIndicatorSeriesPrecision(indicator) {
    if (isValid(this._symbol)) {
      ...
      const synchronize = (indicator2) => {
        switch (indicator2.series) {
          case "price": {
            indicator2.setSeriesPrecision(pricePrecision);
            break;
          }
          case "volume": {
            indicator2.setSeriesPrecision(volumePrecision);
            break;
          }
          default: {
            break;
      ...
      if (isValid(indicator)) {
        ...
        this._indicators.forEach((paneIndicators) => {
          paneIndicators.forEach((indicator2) => {
            synchronize(indicator2);
          });
        });
      }
    }
  ...
  getOverlaysByPaneId(paneId) {
    if (!isString(paneId)) {
      let overlays2 = [];
      this._overlays.forEach((paneOverlays) => {
        overlays2 = overlays2.concat(paneOverlays);
      });
      return overlays2;
    }
  ...
  addOverlays(os, appointPaneFlags) {
    ...
    const ids = os.map((create, index) => {
      if (isValid(create.id)) {
        ...
        for (const item of this._overlays) {
          const overlays2 = item[1];
          const overlay = overlays2.find((o) => o.id === create.id);
          if (isValid(overlay)) {
            findOverlay = overlay;
            break;
          }
      ...
      if (isValid(OverlayClazz)) {
        ...
        create.id = id;
        create.groupId ?? (create.groupId = id);
        const zLevel = this.getOverlaysByPaneId(paneId).length;
        create.zLevel ?? (create.zLevel = zLevel);
    ...
    if (updatePaneIds.length > 0) {
      ...
      updatePaneIds.forEach((paneId) => {
        this._chart.updatePane(1 /* Overlay */, paneId);
      });
      this._chart.updatePane(1 /* Overlay */, PaneIdConstants.X_AXIS);
    }
    return ids;
  }
  ...
  setClickOverlayInfo(info, processOnSelectedEvent, processOnDeselectedEvent) {
    ...
    if (overlay?.id !== infoOverlay?.id || figureType !== info.figureType || figureIndex !== info.figureIndex) {
      ...
      if (overlay?.id !== infoOverlay?.id) {
        ...
        this._chart.updatePane(1 /* Overlay */, info.paneId);
...
var Canvas = class {
    ...
    isSupportedDevicePixelContentBox().then((result) => {
      ...
      if (result) {
        this._resizeObserver = new ResizeObserver((entries) => {
          const entry = entries.find((entry2) => entry2.target === this._element);
  ...
  _executeListener(fn) {
    if (this._requestAnimationId === DEFAULT_REQUEST_ID) {
      this._requestAnimationId = requestAnimationFrame2(() => {
...
var Widget = class extends Eventful {
  ...
  update(level) {
    this.updateImp(this._container, this._bounding, level ?? 3 /* Drawer */);
  }
  destroy() {
...
function drawCircle(ctx, attrs, styles2) {
  ...
  const solid = (style === "fill" || styles2.style === "stroke_fill") && (!isString(color) || !isTransparent(color));
  if (solid) {
    ctx.fillStyle = color;
...
// src/extension/figure/rect.ts
function checkCoordinateOnRect(coordinate, attrs) {
  let rects = [];
  rects = rects.concat(attrs);
  for (const rect4 of rects) {
    let x = rect4.x;
    let width = rect4.width;
    if (width < DEVIATION * 2) {
      x -= DEVIATION;
      width = DEVIATION * 2;
    }
    let y = rect4.y;
    let height = rect4.height;
...
// src/extension/figure/text.ts
function getTextRect(attrs, styles2) {
  const { size = 12, paddingLeft = 0, paddingTop = 0, paddingRight = 0, paddingBottom = 0, weight = "normal", family } = styles2;
  const { x, y, text: text2, align = "left", baseline = "top", width: w, height: h } = attrs;
  const width = w ?? paddingLeft + calcTextWidth(text2, size, weight, family) + paddingRight;
...
function checkCoordinateOnText(coordinate, attrs, styles2) {
  ...
  for (const text2 of texts) {
    const { x, y, width, height } = getTextRect(text2, styles2);
    if (coordinate.x >= x && coordinate.x <= x + width && coordinate.y >= y && coordinate.y <= y + height) {
      return true;
    }
  }
...
function drawText(ctx, attrs, styles2) {
  ...
  const rects = texts.map((text2) => getTextRect(text2, styles2));
  drawRect(ctx, rects, { ...styles2, color: styles2.backgroundColor });
  ctx.textAlign = "left";
  ctx.textBaseline = "top";
  ctx.font = createFont(size, weight, family);
  ctx.fillStyle = color;
  texts.forEach((text2, index) => {
    const rect4 = rects[index];
    ctx.fillText(text2.text, rect4.x + paddingLeft, rect4.y + paddingTop, rect4.width - paddingLeft - paddingRight);
  });
}
var text = {
  name: "text",
...
function checkCoordinateOnArc(coordinate, attrs) {
  ...
  for (const arc2 of arcs) {
    if (Math.abs(getDistance(coordinate, arc2) - arc2.r) < DEVIATION) {
      const { r, startAngle, endAngle } = arc2;
      const startCoordinateX = r * Math.cos(startAngle) + arc2.x;
      const startCoordinateY = r * Math.sin(startAngle) + arc2.y;
      const endCoordinateX = r * Math.cos(endAngle) + arc2.x;
      const endCoordinateY = r * Math.sin(endAngle) + arc2.y;
...
function drawArc(ctx, attrs, styles2) {
  let arcs = [];
  arcs = arcs.concat(attrs);
  const { style = "solid", size = 1, color = "currentColor", dashedValue = [2, 2] } = styles2;
  ctx.lineWidth = size;
  ctx.strokeStyle = color;
  if (style === "dashed") {
    ctx.setLineDash(dashedValue);
...
function drawPath(ctx, attrs, styles2) {
  ...
  const { lineWidth = 1, color = "currentColor" } = styles2;
  ctx.lineWidth = lineWidth;
  ctx.strokeStyle = color;
  ctx.setLineDash([]);
  paths.forEach(({ x, y, path: path2 }) => {
    const commands = path2.match(/[MLHVCSQTAZ][^MLHVCSQTAZ]*/gi);
    if (isValid(commands)) {
      const offsetX = x;
      const offsetY = y;
      ctx.beginPath();
...
var figures = {};
var extensions3 = [circle_default2, line_default, polygon_default, rect_default2, text_default, arc_default, path_default];
extensions3.forEach((figure) => {
...
var GridView = class extends View {
  drawImp(ctx) {
    ...
    const styles2 = chart.getStyles().grid;
    const show = styles2.show;
    if (show) {
      ctx.save();
      ctx.globalCompositeOperation = "destination-over";
      const horizontalStyles = styles2.horizontal;
      ...
      const verticalStyles = styles2.vertical;
...
var CandleBarView = class extends ChildrenView {
  ...
  drawImp(ctx) {
    ...
    if (candleBarOptions !== null) {
      const { type, styles: styles2 } = candleBarOptions;
      let ohlcSize = 0;
      let halfOhlcSize = 0;
      ...
      this.eachChildren((visibleData, barSpace) => {
        ...
        if (isValid(current)) {
          ...
          const comparePrice = styles2.compareRule === "current_open" ? open : prev?.close ?? close;
          const colors = [];
          ...
          rects.forEach((rect4) => {
            ...
            this.createFigure(rect4, handler ?? void 0)?.draw(ctx);
          });
        }
      });
    }
...
var IndicatorView = class extends CandleBarView {
  getCandleBarOptions() {
    ...
    if (!yAxis.isInCandle()) {
      const chartStore = pane.getChart().getChartStore();
      const indicators2 = chartStore.getIndicatorsByPaneId(pane.getId());
        ...
        if (!isCover) {
          ...
          lines.forEach((items) => {
            if (items.length > 1) {
              ...
              mergeLines.forEach(({ coordinates, styles: styles2 }) => {
                this.createFigure({
                  name: "line",
                  attrs: { coordinates },
                  styles: styles2
...
var CrosshairLineView = class extends View {
  drawImp(ctx) {
    ...
    const styles2 = chartStore.getStyles().crosshair;
  ...
  _drawLine(ctx, coordinates, styles2) {
    if (styles2.show) {
      const lineStyles = styles2.line;
      if (lineStyles.show) {
        this.createFigure({
          name: "line",
          attrs: { coordinates },
...
var IndicatorTooltipView = class extends View {
  ...
  drawIndicatorTooltip(ctx, left, top, maxWidth) {
    const pane = this.getWidget().getPane();
    const chartStore = pane.getChart().getChartStore();
    const styles2 = chartStore.getStyles().indicator;
    const tooltipStyles = styles2.tooltip;
  ...
  drawStandardTooltipLegends(ctx, legends, coordinate, left, prevRowHeight, maxWidth, styles2) {
    if (legends.length > 0) {
      const { marginLeft, marginTop, marginRight, marginBottom, size, family, weight } = styles2;
      ctx.font = createFont(size, weight, family);
  ...
  isDrawTooltip(crosshair, styles2) {
    const showRule = styles2.showRule;
  ...
  getIndicatorTooltipData(indicator) {
    ...
    if (indicator.visible) {
      ...
      eachFigures(indicator, dataIndex, styles2, (figure, figureStyles) => {
    ...
    if (isFunction(indicator.createTooltipDataSource)) {
      ...
      if (isValid(customLegends) && indicator.visible) {
        ...
        const color = styles2.tooltip.legend.color;
...
var OverlayView = class extends View {
    ...
    this.registerEvent("mouseMoveEvent", (event) => {
      ...
      if (overlay !== null) {
        if (checkOverlayFigureEvent("onPressedMoveEnd", figure)) {
          overlay.onPressedMoveEnd?.({ chart, overlay, figure: figure ?? void 0, ...event });
        }
      }
      ...
      if (overlay !== null) {
        if (checkOverlayFigureEvent("onPressedMoving", figure)) {
          if (!overlay.lock) {
            ...
            let prevented = false;
            overlay.onPressedMoving?.({ chart, overlay, figure: figure ?? void 0, ...event, preventDefault: () => {
              prevented = true;
            } });
            if (prevented) {
  ...
  _processOverlayMouseEnterEvent(overlay, figure, event) {
    if (isFunction(overlay.onMouseEnter) && checkOverlayFigureEvent("onMouseEnter", figure)) {
      overlay.onMouseEnter({ chart: this.getWidget().getPane().getChart(), overlay, figure: figure ?? void 0, ...event });
      return true;
    }
    return false;
  }
  _processOverlayMouseLeaveEvent(overlay, figure, event) {
    if (isFunction(overlay.onMouseLeave) && checkOverlayFigureEvent("onMouseLeave", figure)) {
      overlay.onMouseLeave({ chart: this.getWidget().getPane().getChart(), overlay, figure: figure ?? void 0, ...event });
      return true;
    }
    return false;
  }
  _processOverlaySelectedEvent(overlay, figure, event) {
    if (checkOverlayFigureEvent("onSelected", figure)) {
      overlay.onSelected?.({ chart: this.getWidget().getPane().getChart(), overlay, figure: figure ?? void 0, ...event });
      return true;
    }
    return false;
  }
  _processOverlayDeselectedEvent(overlay, figure, event) {
    if (checkOverlayFigureEvent("onDeselected", figure)) {
      overlay.onDeselected?.({ chart: this.getWidget().getPane().getChart(), overlay, figure: figure ?? void 0, ...event });
      return true;
    }
    return false;
  }
  _figureMouseMoveEvent(overlay, figureType, figureIndex, figure) {
    return (event) => {
      ...
      if (check) {
        let prevented = false;
        overlay.onMouseMove?.({ chart: pane.getChart(), overlay, figure, ...event, preventDefault: () => {
          prevented = true;
        } });
        if (prevented) {
          this.getWidget().setForceCursor(null);
  ...
  _figureMouseRightClickEvent(overlay, _figureType, _figureIndex, figure) {
    return (event) => {
      if (checkOverlayFigureEvent("onRightClick", figure)) {
        let prevented = false;
        overlay.onRightClick?.({ chart: this.getWidget().getPane().getChart(), overlay, figure, ...event, preventDefault: () => {
          prevented = true;
        } });
        if (!prevented) {
  ...
  _coordinateToPoint(o, coordinate) {
    ...
    if (this.coordinateToPointTimestampDataIndexFlag()) {
      ...
      const timestamp = chartStore.dataIndexToTimestamp(dataIndex) ?? void 0;
  ...
  drawImp(ctx) {
    const overlays2 = this.getCompleteOverlays();
  ...
  _drawOverlay(ctx, overlay) {
    ...
    if (coordinates.length > 0) {
      const figures2 = [].concat(this.getFigures(overlay, coordinates));
      this.drawFigures(
        ctx,
        overlay,
        figures2
  ...
  drawFigures(ctx, overlay, figures2) {
    const defaultStyles = this.getWidget().getPane().getChart().getStyles().overlay;
    figures2.forEach((figure, figureIndex) => {
      const { type, styles: styles2, attrs } = figure;
      ...
      attrsArray.forEach((ats) => {
        ...
        const ss = { ...defaultStyles[type], ...overlay.styles?.[type], ...styles2 };
        this.createFigure({
          name: type,
          attrs: ats,
          styles: ss
  ...
  drawDefaultFigures(ctx, overlay, coordinates) {
    if (overlay.needDefaultPointFigure) {
      ...
      if (hoverOverlayInfo.overlay?.id === overlay.id && hoverOverlayInfo.figureType !== "none" || clickOverlayInfo.overlay?.id === overlay.id && clickOverlayInfo.figureType !== "none") {
        ...
        const styles2 = overlay.styles;
        const pointStyles = { ...defaultStyles.point, ...styles2?.point };
...
var CandleAreaView = class extends ChildrenView {
  constructor() {
    ...
    this._animation = new Animation({ iterationCount: Infinity }).doFrame((time) => {
      ...
      const pane = this.getWidget().getPane();
      pane.getChart().updatePane(0 /* Main */, pane.getId());
    });
  }
  drawImp(ctx) {
    const widget = this.getWidget();
    ...
    const bounding = widget.getBounding();
    const yAxis = pane.getAxisComponent();
    const styles2 = chart.getStyles().candle.area;
    const coordinates = [];
    ...
    this.eachChildren((data) => {
      ...
      const value = kLineData?.[styles2.value];
      if (isNumber(value)) {
    ...
    const pointStyles = styles2.point;
...
// src/view/CandleHighLowPriceView.ts
var CandleHighLowPriceView = class extends View {
  ...
  _drawMark(ctx, text2, coordinate, offsets, styles2) {
...
// src/view/CandleLastPriceLineView.ts
...
var CandleTooltipView = class extends IndicatorTooltipView {
  drawImp(ctx) {
    ...
    if (isValid(crosshair.kLineData)) {
      const bounding = widget.getBounding();
      const styles2 = chartStore.getStyles();
      const candleStyles = styles2.candle;
      const indicatorStyles = styles2.indicator;
  ...
  _drawCandleStandardTooltip(ctx, left, top, maxWidth) {
    const chartStore = this.getWidget().getPane().getChart().getChartStore();
    const styles2 = chartStore.getStyles().candle;
    const tooltipStyles = styles2.tooltip;
    const tooltipLegendStyles = tooltipStyles.legend;
    let prevRowHeight = 0;
    const coordinate = { x: left, y: top };
    ...
    if (this.isDrawTooltip(crosshair, tooltipStyles)) {
      ...
      if (tooltipTitleStyles.show) {
        ...
        const text2 = formatTemplateString(tooltipTitleStyles.template, { ...chartStore.getSymbol(), period: `${span}${i18n(type, chartStore.getLocale())}` });
        const color = tooltipTitleStyles.color;
    ...
    if (isDrawCandleTooltip || isDrawIndicatorTooltip) {
      ...
      if (isDrawCandleTooltip) {
        ...
        candleLegends.forEach((data) => {
          const title = data.title;
          const value = data.value;
          const text2 = `${title.text}${value.text}`;
          const labelWidth = ctx.measureText(text2).width + baseLegendMarginLeft + baseLegendMarginRight;
      ...
      if (isDrawIndicatorTooltip) {
        ...
        indicators2.forEach((indicator) => {
          ...
          tooltipDataLegends.forEach((data) => {
            ...
            const value = data.value;
            const text2 = `${title.text}${value.text}`;
            const textWidth = ctx.measureText(text2).width + indicatorLegendMarginLeft + indicatorLegendMarginRight;
  ...
  _getCandleTooltipLegends() {
    ...
    const legends = isFunction(template) ? template({ prev, current, next: dataList[dataIndex + 1] ?? null }, styles2) : template;
...
var CrosshairFeatureView = class extends View {
  ...
  drawImp(ctx) {
    ...
    if (isString(crosshair.paneId) && crosshair.paneId === pane.getId() && yAxis.isInCandle()) {
      const styles2 = chartStore.getStyles().crosshair;
      const features = styles2.horizontal.features;
      if (styles2.show && styles2.horizontal.show && features.length > 0) {
        ...
        features.forEach((feature) => {
          ...
          if (type === "icon_font") {
            ...
            this.createFigure({
              name: "path",
              attrs: { path: path2.path, x, y: y + marginTop + paddingTop - size / 2, width: size, height: size },
              styles: {
                style: path2.style,
                lineWidth: path2.lineWidth,
                color: finalColor
...
var AxisView = class extends View {
  drawImp(ctx, extend) {
    ...
    const styles2 = this.getAxisStyles(pane.getChart().getStyles());
    if (styles2.show) {
      ...
      if (!extend[0]) {
        ...
        if (styles2.tickLine.show) {
          const lines = this.createTickLines(ticks, bounding, styles2);
          lines.forEach((line2) => {
            this.createFigure({
              name: "line",
              attrs: line2,
        ...
        if (styles2.tickText.show) {
          const texts = this.createTickTexts(ticks, bounding, styles2);
          this.createFigure({
            name: "text",
            attrs: texts,
...
// src/view/YAxisView.ts
var YAxisView = class extends AxisView {
  getAxisStyles(styles2) {
    return styles2.yAxis;
  }
  createAxisLine(bounding, styles2) {
    const yAxis = this.getWidget().getPane().getAxisComponent();
    const size = styles2.axisLine.size;
    let x = 0;
    if (yAxis.isFromZero()) {
      x = 0;
    } else {
  ...
  createTickLines(ticks, bounding, styles2) {
    const yAxis = this.getWidget().getPane().getAxisComponent();
    const axisLineStyles = styles2.axisLine;
    const tickLineStyles = styles2.tickLine;
    let startX = 0;
    let endX = 0;
    if (yAxis.isFromZero()) {
      startX = 0;
  ...
  createTickTexts(ticks, bounding, styles2) {
    const yAxis = this.getWidget().getPane().getAxisComponent();
    const axisLineStyles = styles2.axisLine;
    const tickLineStyles = styles2.tickLine;
    const tickTextStyles = styles2.tickText;
    let x = 0;
    if (yAxis.isFromZero()) {
...
// src/view/CandleLastPriceLabelView.ts
    ...
    if (priceMarkStyles.show && lastPriceMarkStyles.show && lastPriceMarkTextStyles.show) {
      ...
      if (isValid(data)) {
        ...
        lastPriceMarkStyles.extendTexts.forEach((item, index) => {
          const text2 = formatExtendText({ type: "last_price", data, index });
          if (text2.length > 0 && item.show) {
            ...
            textWidth = Math.max(textWidth, item.paddingLeft + calcTextWidth(text2, item.size, item.weight, item.family) + item.paddingRight);
            textFigures.push({
              name: "text",
              attrs: {
                x,
...
// src/view/IndicatorLastValueView.ts
        ...
        if (isValid(data) && indicator.visible) {
          ...
          eachFigures(indicator, dataIndex, defaultStyles, (figure, figureStyles) => {
            ...
            if (isNumber(value)) {
              ...
              if (indicator.shouldFormatBigNumber) {
                text2 = formatter.formatBigNumber(text2);
              }
              text2 = decimalFold.format(thousandsSeparator.format(text2));
              let x = 0;
              let textAlign = "left";
              if (yAxis.isFromZero()) {
                x = 0;
    ...
    if (overlay.needDefaultYAxisFigure && overlay.id === clickOverlayInfo.overlay?.id && clickOverlayInfo.paneId === pane.getId()) {
      ...
      coordinates.forEach((coordinate, index) => {
        ...
        if (isNumber(point.value)) {
          ...
          const text2 = decimalFold.format(thousandsSeparator.format(formatPrecision(point.value, chartStore.getSymbol()?.pricePrecision ?? SymbolDefaultPrecisionConstants.PRICE)));
          figures2.push({ type: "text", attrs: { x, y: coordinate.y, text: text2, align: textAlign, baseline: "middle" }, ignoreEvent: true });
        }
      });
      if (coordinates.length > 1) {
        figures2.unshift({ type: "rect", attrs: { x: 0, y: topY, width: bounding.width, height: bottomY - topY }, ignoreEvent: true });
      }
    }
    return figures2;
  }
...
// src/view/CrosshairHorizontalLabelView.ts
var CrosshairHorizontalLabelView = class extends View {
  drawImp(ctx) {
    ...
    if (isString(crosshair.paneId) && this.compare(crosshair, pane.getId())) {
      const styles2 = chartStore.getStyles().crosshair;
      if (styles2.show) {
        const directionStyles = this.getDirectionStyles(styles2);
        const textStyles = directionStyles.text;
        if (directionStyles.show && textStyles.show) {
          const bounding = widget.getBounding();
          const axis = pane.getAxisComponent();
          const text2 = this.getText(crosshair, chartStore, axis);
  ...
  getText(crosshair, chartStore, axis) {
    ...
    if (yAxis.isInCandle()) {
      ...
      const indicators2 = chartStore.getIndicatorsByPaneId(crosshair.paneId);
      indicators2.forEach((indicator) => {
  ...
  getTextAttrs(text2, _textWidth, crosshair, bounding, axis, _styles) {
    const yAxis = axis;
    let x = 0;
    let textAlign = "left";
    if (yAxis.isFromZero()) {
...
var TICK_COUNT = 8;
var YAxisImp = class _YAxisImp extends AxisImp {
  ...
  createRangeImp() {
    ...
    const indicators2 = chartStore.getIndicatorsByPaneId(paneId);
    indicators2.forEach((indicator) => {
    ...
    visibleRangeDataList.forEach((visibleData) => {
      ...
      indicators2.forEach(({ result, figures: figures2 }) => {
        const data2 = result[dataIndex] ?? {};
        figures2.forEach((figure) => {
          const value = data2[figure.key];
          if (isNumber(value)) {
            min = Math.min(min, value);
            max = Math.max(max, value);
          }
  ...
  createTicksImp() {
    ...
    if (displayRange >= 0) {
      ...
      const precision2 = getPrecision(interval);
      const first = round(Math.ceil(displayFrom / interval) * interval, precision2);
      const last = round(Math.floor(displayTo / interval) * interval, precision2);
      let n = 0;
      let f = first;
      if (interval !== 0) {
        while (f <= last) {
          const v = f.toFixed(precision2);
    ...
    const thousandsSeparator = chartStore.getThousandsSeparator();
    const decimalFold = chartStore.getDecimalFold();
    const textHeight = styles2.xAxis.tickText.size;
    let validY = NaN;
    ticks.forEach(({ value }) => {
  ...
  getAutoSize() {
    ...
    const chartStore = chart.getChartStore();
    const styles2 = chartStore.getStyles();
    const yAxisStyles = styles2.yAxis;
    ...
    const priceMarkStyles = styles2.candle.priceMark;
    ...
    const crosshairStyles = styles2.crosshair;
    ...
    if (lastPriceMarkTextVisible || crosshairHorizontalTextVisible) {
      ...
      if (crosshairHorizontalTextVisible) {
        ...
        let precision = 2;
        if (this.isInCandle()) {
          const lastValueMarkStyles = styles2.indicator.lastValueMark;
        ...
        crosshairHorizontalTextWidth += crosshairStyles.horizontal.text.paddingLeft + crosshairStyles.horizontal.text.paddingRight + crosshairStyles.horizontal.text.borderSize * 2 + calcTextWidth(
          valueText,
  ...
  static extend(template) {
    class Custom extends _YAxisImp {
      constructor(parent) {
...
// src/extension/y-axis/normal.ts
var normal = {
  name: "normal"
};
var normal_default = normal;
...
var percentage_default = percentage;
...
var logarithm_default = logarithm;

// src/extension/y-axis/index.ts
var yAxises = {
  normal: YAxisImp.extend(normal_default),
  percentage: YAxisImp.extend(percentage_default),
  logarithm: YAxisImp.extend(logarithm_default)
};
...
var Pane = class {
  ...
  update(level) {
    ...
    this.updateImp(level ?? 3 /* Drawer */, this._container, this._bounding);
  }
};

// src/pane/DrawPane.ts
...
var XAxisView = class extends AxisView {
  ...
  createTickLines(ticks, _bounding, styles2) {
    const tickLineStyles = styles2.tickLine;
    const axisLineSize = styles2.axisLine.size;
    return ticks.map((tick) => ({
      coordinates: [
        { x: tick.coord, y: 0 },
  ...
  createTickTexts(ticks, _bounding, styles2) {
    const tickTickStyles = styles2.tickText;
    const axisLineSize = styles2.axisLine.size;
    const tickLineLength = styles2.tickLine.length;
    return ticks.map((tick) => ({
      x: tick.coord,
  ...
  getDefaultFigures(overlay, coordinates) {
    ...
    if (overlay.needDefaultXAxisFigure && overlay.id === clickOverlayInfo.overlay?.id) {
      ...
      coordinates.forEach((coordinate, index) => {
        ...
        const point = overlay.points[index];
        if (isNumber(point.timestamp)) {
          const text2 = chartStore.getInnerFormatter().formatDate(point.timestamp, "YYYY-MM-DD HH:mm", "crosshair");
          figures2.push({ type: "text", attrs: { x: coordinate.x, y: 0, text: text2, align: "center" }, ignoreEvent: true });
        }
      });
      if (coordinates.length > 1) {
        figures2.unshift({ type: "rect", attrs: { x: leftX, y: 0, width: rightX - leftX, height: widget.getBounding().height }, ignoreEvent: true });
      }
    }
    return figures2;
  }
...
var CrosshairVerticalLabelView = class extends CrosshairHorizontalLabelView {
  ...
  getTextAttrs(text2, textWidth, crosshair, bounding, _axis, styles2) {
    const x = crosshair.realX;
    let optimalX = 0;
    let align = "center";
...
var XAxisImp = class _XAxisImp extends AxisImp {
  ...
  getAutoSize() {
    const styles2 = this.getParent().getChart().getStyles();
    const xAxisStyles = styles2.xAxis;
  ...
  static extend(template) {
    class Custom extends _XAxisImp {
      constructor(parent) {
        super(parent, template);
      }
    }
...
// src/extension/x-axis/normal.ts
var normal2 = {
  name: "normal"
};
var normal_default2 = normal2;

// src/extension/x-axis/index.ts
var xAxises = {
  normal: XAxisImp.extend(normal_default2)
};
...
var SeparatorWidget = class extends Widget {
  ...
  _mouseDownEvent(event) {
    ...
    if (this._topPane.getOptions().state === "minimize") {
      const index = drawPanes.findIndex((pane2) => pane2.getId() === this._topPane?.getId());
      for (let i = index - 1; i > -1; i--) {
        const pane2 = drawPanes[i];
        if (pane2.getOptions().state !== "minimize") {
          this._topPane = pane2;
          break;
        }
      }
    ...
    if (this._bottomPane.getOptions().state === "minimize") {
      const index = drawPanes.findIndex((pane2) => pane2.getId() === this._bottomPane?.getId());
      for (let i = index + 1; i < drawPanes.length; i++) {
        const pane2 = drawPanes[i];
        if (pane2.getOptions().state !== "minimize") {
          this._bottomPane = pane2;
          break;
        }
      }
    }
  ...
  _mouseEnterEvent() {
    ...
    if (bottomPane.getOptions().dragEnabled) {
      const chart = pane.getChart();
      const styles2 = chart.getStyles().separator;
  ...
  updateImp(container, _bounding, level) {
    if (level === 4 /* All */ || level === 2 /* Separator */) {
      const styles2 = this.getPane().getChart().getStyles().separator;
      container.style.top = `${-Math.floor((REAL_SEPARATOR_HEIGHT - styles2.size) / 2)}px`;
      container.style.height = `${REAL_SEPARATOR_HEIGHT}px`;
    }
  }
};

// src/pane/SeparatorPane.ts
var SeparatorPane = class extends Pane {
  ...
  getImage(_includeOverlay) {
    const { width, height } = this.getBounding();
    const styles2 = this.getChart().getStyles().separator;
    const canvas = createDom("canvas", {
      width: `${width}px`,
      height: `${height}px`,
      boxSizing: "border-box"
...
var Event = class {
  ...
  mouseWheelHortEvent(_, distance4) {
    const store = this._chart.getChartStore();
    store.startScroll();
    store.scroll(distance4);
    return true;
  }
      ...
      switch (name) {
        case WidgetNameConstants.MAIN: {
          ...
          if (this._startScrollCoordinate !== null) {
            ...
            const distance4 = event.x - this._startScrollCoordinate.x;
            let v = distance4 / (time > 0 ? time : 1) * 20;
            if (time < 200 && Math.abs(v) > 0) {
              const store = this._chart.getChartStore();
              const flingScroll = () => {
                this._flingScrollRequestId = requestAnimationFrame2(() => {
                  store.startScroll();
                  store.scroll(v);
                  v = v * (1 - 0.025);
                  if (Math.abs(v) < 1) {
  ...
  tapEvent(e) {
    ...
    if (widget !== null) {
      ...
      if (widget.getName() === WidgetNameConstants.MAIN) {
        const event2 = this._makeWidgetEvent(e, widget);
        const chartStore = this._chart.getChartStore();
        if (result) {
          this._touchCancelCrosshair = true;
          this._touchCoordinate = null;
          ...
          if (!this._touchCancelCrosshair && !this._touchZoomed) {
            this._touchCoordinate = { x: event2.x, y: event2.y };
            chartStore.setCrosshair({ x: event2.x, y: event2.y, paneId: pane?.getId() }, { notInvalidate: true });
            consumed = true;
          }
          this._touchCancelCrosshair = false;
        }
  ...
  _processMainScrollingEvent(widget, event) {
    if (this._startScrollCoordinate !== null) {
      ...
      if (this._prevYAxisRange !== null && !yAxis.getAutoCalcTickFlag() && yAxis.scrollZoomEnabled) {
        ...
        let distance5 = 0;
        if (yAxis.reverse) {
          distance5 = this._startScrollCoordinate.y - event.y;
        } else {
          distance5 = event.y - this._startScrollCoordinate.y;
        }
        const bounding = widget.getBounding();
        const scale = distance5 / bounding.height;
      ...
      const distance4 = event.x - this._startScrollCoordinate.x;
      this._chart.getChartStore().scroll(distance4);
    }
  }
  ...
  _findWidgetByEvent(event) {
    ...
    for (const items of separatorPanes) {
      const pane2 = items[1];
      const bounding = pane2.getBounding();
      ...
      if (x >= bounding.left && x <= bounding.left + bounding.width && y >= top && y <= top + REAL_SEPARATOR_HEIGHT) {
        return { pane: pane2, widget: pane2.getWidget() };
      }
    }
...
var ChartImp = class {
  ...
  _layout() {
    ...
    if (forceMeasureWidth) {
      ...
      const separatorFill = styles2.separator.fill;
  ...
  setOffsetRightDistance(distance4) {
    this._chartStore.setOffsetRightDistance(distance4, true);
  }
  getOffsetRightDistance() {
  ...
  setMaxOffsetLeftDistance(distance4) {
    ...
    this._chartStore.setMaxOffsetLeftDistance(distance4);
  }
  setMaxOffsetRightDistance(distance4) {
    if (distance4 < 0) {
    ...
    this._chartStore.setMaxOffsetRightDistance(distance4);
  }
  setLeftMinVisibleBarCount(barCount) {
    if (barCount < 0) {
  ...
  createIndicator(value, isStack, paneOptions) {
    ...
    if (result) {
      ...
      if (!isValid(this.getDrawPaneById(paneOpts.id))) {
        ...
        paneOpts.height ?? (paneOpts.height = PANE_DEFAULT_HEIGHT);
        shouldSort = true;
      }
    ...
    if (removed) {
      ...
      paneIds.forEach((paneId) => {
        if (!this._chartStore.hasIndicators(paneId)) {
          const index = this._drawPanes.findIndex((pane2) => pane2.getId() === paneId);
  ...
  createOverlay(value) {
    ...
    const ids = this._chartStore.addOverlays(overlays2, appointPaneFlags);
    if (isArray(value)) {
      return ids;
    }
    return ids[0];
          ...
          if (isValid(options.state) && currentPane.getOptions().state !== options.state) {
            ...
            switch (state) {
              ...
              case "minimize": {
                ...
                if (this._recalculatePaneHeight(
                  currentPane,
                  PANE_MIN_HEIGHT,
                  changeHeight
                )) {
  ...
  scrollByDistance(distance4, animationDuration) {
    ...
    if (duration > 0) {
      ...
      animation.doFrame((frameTime) => {
        const progressDistance = distance4 * (frameTime / duration);
        this._chartStore.scroll(progressDistance);
      });
      animation.start();
    } else {
      this._chartStore.scroll(distance4);
    }
  }
  scrollToRealTime(animationDuration) {
    ...
    const distance4 = difBarCount * barSpace;
    this.scrollByDistance(distance4, animationDuration);
  }
  scrollToDataIndex(dataIndex, animationDuration) {
    const distance4 = (this._chartStore.getLastBarRightSideDiffBarCount() + (this.getDataList().length - 1 - dataIndex)) * this._chartStore.getBarSpace().bar;
    this.scrollByDistance(distance4, animationDuration);
  }
  ...
  zoomAtCoordinate(scale, coordinate, animationDuration) {
    ...
    if (duration > 0) {
      ...
      animation.doFrame((frameTime) => {
        ...
        const scale2 = (progressBarSpace - prevProgressBarSpace) / this._chartStore.getBarSpace().bar * SCALE_MULTIPLIER;
        this._chartStore.zoom(scale2, coordinate ?? null, "main");
...
// src/pro/overlays.ts
function rotatePoint3(p, center, angle) {
...
function extendLine3(coords, bounding) {
  if (coords.length > 1) {
    let end;
    if (coords[0].x === coords[1].x && coords[0].y !== coords[1].y) {
      end = coords[0].y < coords[1].y ? { x: coords[0].x, y: bounding.height } : { x: coords[0].x, y: 0 };
...
var arrow2 = {
  ...
  createPointFigures: ({ coordinates }) => {
    if (coordinates.length > 1) {
      ...
      const a1 = rotatePoint3({ x: coordinates[1].x - 8, y: coordinates[1].y + 4 }, coordinates[1], angle);
      const a2 = rotatePoint3({ x: coordinates[1].x - 8, y: coordinates[1].y - 4 }, coordinates[1], angle);
      return [
        { type: "line", attrs: { coordinates } },
...
var circle3 = {
  ...
  createPointFigures: ({ coordinates }) => {
    if (coordinates.length > 1) {
      const r = getDistance2(coordinates[0], coordinates[1]);
  ...
  createPointFigures: ({ coordinates, overlay }) => {
    ...
    if (coordinates.length > 1) {
      ...
      levels.forEach((l) => {
        ...
        const price = (points[1].value + valueDiff * l).toFixed(2);
...
var fibonacciSpiral2 = {
  ...
  createPointFigures: ({ coordinates, bounding }) => {
    if (coordinates.length > 1) {
      const baseR = getDistance2(coordinates[0], coordinates[1]) / Math.sqrt(24);
      const dir = coordinates[1].x > coordinates[0].x ? 0 : 1;
      const si = utils.getLinearSlopeIntercept(coordinates[0], coordinates[1]);
      let angle;
      if (si) {
      ...
      const c1 = rotatePoint3({ x: coordinates[0].x - baseR, y: coordinates[0].y }, coordinates[0], angle);
      const c2 = rotatePoint3({ x: coordinates[0].x - baseR, y: coordinates[0].y - baseR }, coordinates[0], angle);
      const arcs = [
      ...
      for (let i = 2; i < 9; i++) {
        ...
        const center = rotatePoint3({ x: cx, y: cy }, coordinates[0], angle);
        arcs.push({ ...center, r, startAngle: sa, endAngle: sa + Math.PI / 2 });
      }
      return [
        { type: "arc", attrs: arcs },
        { type: "line", attrs: extendLine3(coordinates, bounding) }
      ];
    }
    return [];
  }
};
var fibonacciSpeedResistanceFan2 = {
  name: "fibonacciSpeedResistanceFan",
  totalStep: 3,
  needDefaultPointFigure: true,
  needDefaultXAxisFigure: true,
  ...
  createPointFigures: ({ coordinates, bounding }) => {
    ...
    if (coordinates.length > 1) {
      ...
      levels.forEach((l) => {
        ...
        fanLines = fanLines.concat(extendLine3([coordinates[0], { x, y: coordinates[1].y }], bounding));
        fanLines = fanLines.concat(extendLine3([coordinates[0], { x: coordinates[1].x, y }], bounding));
  ...
  createPointFigures: ({ coordinates, overlay }) => {
    ...
    if (coordinates.length > 2) {
      ...
      levels.forEach((l) => {
        ...
        const price = (points[2].value + valueDiff * l).toFixed(2);
...
var threeWaves2 = makeWaveOverlay("threeWaves", 5);
var fiveWaves2 = makeWaveOverlay("fiveWaves", 7);
var eightWaves2 = makeWaveOverlay("eightWaves", 10);
var anyWaves2 = makeWaveOverlay("anyWaves", Number.MAX_SAFE_INTEGER);
var abcd2 = {
  name: "abcd",
  totalStep: 5,
  needDefaultPointFigure: true,
...
var measure2 = {
  ...
  createPointFigures: ({ coordinates, overlay, bounding }) => {
    if (coordinates.length > 1) {
      ...
      const text2 = `${diff.toFixed(2)} (${pct.toFixed(2)}%)`;
      const figs = [
        {
          type: "polygon",
          attrs: {
      ...
      const textWidth = Math.max(utils.calcTextWidth(text2), 80) + 24;
      const textHeight = 28;
      let textY;
      if (isDown) {
      ...
      figs.push({
        type: "text",
        attrs: { x: mid.x, y: textY + 8, text: text2, align: "center" },
        styles: { paddingLeft: 0, paddingTop: 0, paddingRight: 0, paddingBottom: 0, backgroundColor: "none", family: "Space Grotesk, sans-serif" }
      });
      return figs;
    }
...
var DrawingBar = class {
  constructor(options) {
    ...
    groups.forEach(([groupName, tools]) => {
      ...
      if (tools.length === 1) {
        ...
        arrow3.className = "equicharts-drawing-bar__arrow";
        arrow3.innerHTML = '<svg width="6" height="6" viewBox="0 0 4 6"><path d="M1 0.16C0.83-0.05 0.43-0.05 0.18 0.16C-0.06 0.37-0.06 0.72 0.18 0.93L2.61 3.03L0.26 5.07C0.01 5.28 0.01 5.63 0.26 5.84C0.51 6.05 0.9 6.05 1.15 5.84L3.82 3.53C4.02 3.36 4.05 3.09 3.92 2.88C3.93 2.73 3.87 2.58 3.74 2.47L1 0.16Z" fill="currentColor"/></svg>';
        item.appendChild(arrow3);
  ...
  _toggleSubmenu(groupName, tools, anchor, options) {
    ...
    const rect4 = anchor.getBoundingClientRect();
    menu.style.position = "fixed";
    menu.style.left = `${rect4.right + 4}px`;
    menu.style.top = `${rect4.top}px`;
...
var ChartMain = class {
  constructor(options) {
    ...
    this._drawingBar = new DrawingBar({
      chart,
      onDrawingItemClick: (name) => {
        chart.createOverlay(name);
      },
      onModeChange: (mode) => {
        chart.overrideOverlay({ mode });
      },
      onLockChange: (locked) => {
        chart.overrideOverlay({ lock: locked });
      },
      onVisibleChange: (visible) => {
        chart.overrideOverlay({ visible });
      },
      onRemoveClick: () => {
        chart.removeOverlay();
      }
  ...
  getTheme() {
    return this._container.getAttribute("data-theme") ?? "dark";
  }
  setStyles(styles2) {
    this._chart?.setStyles(styles2);
  }
  getStyles() {
    return this._chart?.getStyles();
  }
  setLocale(locale) {
    this._chart?.setLocale(locale);
  }
  getLocale() {
    return this._chart?.getLocale() ?? "en-US";
  }
  setSymbol(symbol) {
  ...
  setPeriod(period) {
    this._chart?.setPeriod({ type: period.timespan, span: period.multiplier });
  }
  getPeriod() {
    return this._chart?.getPeriod();
  }
  setYScrolling(enabled) {
    this._chart?.setYScrolling?.(enabled);
  }
  getYScrolling() {
    return this._chart?.getYScrolling?.() ?? true;
  }
  resize() {
    this._chart?.resize();
  }
  setDataLoader(loader) {
    this._chart?.setDataLoader(loader);
  }
  createOverlay(name, paneId) {
    return this._chart?.createOverlay(paneId ? { name, paneId } : name);
  }
  overrideOverlay(opts) {
    this._chart?.overrideOverlay(opts);
  }
  removeOverlay(opts) {
    this._chart?.removeOverlay(opts);
  }
  ...
  removeIndicator(paneId, name) {
    this._chart?.removeIndicator(name ? { paneId, name } : { paneId });
  }
  getConvertPictureUrl(withWatermark, type, bgColor) {
  ...
  getDrawingBarVisible() {
    return this._drawingBarVisible;
  }
  toggleIndicators() {
  }
  destroy() {
...
//# sourceMappingURL=equicharts.js.map
